...
export function AggregatorContainer() {
	...
	const [isPrivacyEnabled, setIsPrivacyEnabled] = useLocalStorage('VerbSwap-isprivacyenabled', false);
	...
	const handleSwap = () => {
		if (normalizedRoutes.length && normalizedRoutes[0]?.name === 'LlamaZip') {
			if (+normalizedRoutes[0].fromAmount > 11_000 * 10 ** 18) {
				toast({
					title: 'Your size is size. Please use app.verbswap.xyz',
					status: 'warning',
					duration: 10000,
					isClosable: true,
					position: 'top-right',
				...
				<Body showRoutes={finalSelectedFromToken && finalSelectedToToken ? true : false}>
					...
					{degenSizeIsSize ? (
						<Alert status="warning" borderRadius="0.375rem" py="8px" fontSize={'16px'}>
							<AlertIcon />
							Your size is size. Please use app.verbswap.xyz
						</Alert>
					) : null}
					...
					<SwapWrapper>
						{!isConnected ? (
							<Button colorScheme={'green'} onClick={openConnectModal} variant='outline'>
								Connect Wallet
							</Button>
